<template>
  <canvas id="telesalesassigned-chart"></canvas>
</template>

<script>
import { eventBus } from '../../../index';
  import Chart from 'chart.js';
  import telesalesAssignedChart from './telesales-chart.js';

  export default {
    name: 'TelesalesAssigned',
    data() {
      return {
        telesalesAssignedChart: telesalesAssignedChart,
        weeklyTeleSalesA: [],
        weeklyTeleSalesB: [],
        monthlyTeleSalesA: [],
        monthlyTeleSalesB: [],
      ...
      getBitrixDataForTheWeek(data) {
        ...
        for (let index in data) {
         ...
         if (BitrixdateNow >= MondayDateNow) {             
            ...
            //     if (data[index].STATUS_ID === this.$store.state.statusIdD.assigned) {
      ...
      getWeeklyIndicatorData(data) {
        for (let index in data) {          
          if (data[index].period.trim() === "weekly".trim()) {
            this.chartColor = parseInt(data[index].telesales_assigned_lead) + 100;
            telesalesAssignedChart.options.scales.yAxes[0].ticks.max = 
            parseInt(data[index].telesales_assigned_lead) + 100;            
          }
        }  
      },
      getWeeklySalesData(data) {
         ...
         for (let index in data) {
          if (data[index].staff_name.trim().toLowerCase() === "lina".toLowerCase()) {
            telesalesAssignedChart.data.labels[0] = "LINA"; 
            telesalesAssignedChart.data.datasets[0].data[0] = parseInt(data[index].assigned_lead);
            if (parseInt(data[index].assigned_lead) < (this.chartColor - 105)) {
              telesalesAssignedChart.data.datasets[0].backgroundColor[0] = "#bf0006";
            } else if (parseInt(data[index].assigned_lead) <= (this.chartColor - 100)) {
              telesalesAssignedChart.data.datasets[0].backgroundColor[0] = "#0b6623";
            } else if (parseInt(data[index].assigned_lead) >  (this.chartColor - 100)) {
              telesalesAssignedChart.data.datasets[0].backgroundColor[0] = "#4cbb17";
            }          
         }

        //  if (data[index].staff_name.trim().toLowerCase() === "gloria".toLowerCase()) {
        //     telesalesAssignedChart.data.labels[1] = "GLORIA";
        //     telesalesAssignedChart.data.datasets[0].data[1] = parseInt(data[index].assigned_lead);
        //     if (parseInt(data[index].assigned_lead) < (this.chartColor - 105)) {
        //       telesalesAssignedChart.data.datasets[0].backgroundColor[1] = "#bf0006";
        //     } else if (parseInt(data[index].assigned_lead) <= (this.chartColor - 100)) {
        //       telesalesAssignedChart.data.datasets[0].backgroundColor[1] = "#0b6623";
        //     } else if (parseInt(data[index].assigned_lead) > (this.chartColor - 100)) {
        //       telesalesAssignedChart.data.datasets[0].backgroundColor[1] = "#4cbb17";
        //     }          
        //  }
        }            
         this.createChart('telesalesassigned-chart', this.telesalesAssignedChart);
      },
    },
    created() {
